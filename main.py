def timing():
    ...
    try:
        ...
        # منتظر لود داده‌ها باشه
        print("منتظر لود داده‌ها...")
        wait = WebDriverWait(driver, 90)  # انتظار طولانی‌تر
        
        # امتحان روش‌های مختلف برای پیدا کردن داده‌ها
        data_found = False
        
        # روش 1: منتظر وجود حداقل یک ردیف جدول باشه
        try:
            wait.until(
            ...
            print("❌ روش 1 نشد - امتحان روش‌های دیگه...")
        
        # روش 2: چک کردن وجود المنت‌های مشابه
        if not data_found:
            try:
                elements = driver.find_elements(By.CLASS_NAME, 'ds-table-row')
                if elements:
                    print(f"✅ {len(elements)} ردیف با کلاس ds-table-row پیدا شد")
                    data_found = True
                else:
                    elements = driver.find_elements(By.CLASS_NAME, 'ds-dex-table-row')
                    if elements:
                        print(f"✅ {len(elements)} ردیف با کلاس ds-dex-table-row پیدا شد")
                        data_found = True
            except:
                pass
        
        # روش 3: چک کردن محتوای صفحه
        if not data_found:
            try:
                page_source = driver.page_source
                if 'ds-dex-table' in page_source or 'ds-table' in page_source:
                    print("✅ جدول در HTML صفحه وجود داره")
                    data_found = True
                elif 'tbody' in page_source and 'tr' in page_source:
                    print("✅ ساختار جدول در HTML وجود داره")
                    data_found = True
            except:
                pass
        
        if not data_found:
            raise ValueError("داده‌ها لود نشدن")
        ...
        try:
            ...
            if len(data_lines) < 20:  # حداقل 20 خط داده
